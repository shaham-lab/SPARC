...
class GraphConvolutionOld(Layer):
  ...
  def __init__(self,
               ...
               precalc=False,
               **kwargs):
    super(GraphConvolutionOld, self).__init__(**kwargs)
...
class GraphConvolution(Layer):
  ...
  def __init__(self,
    ...
    self.act = act
    self.support = placeholders['support']
    self.eigen_vecs = placeholders['eigen_vecs']
    self.sparse_inputs = sparse_inputs
    self.featureless = featureless
    self.bias = bias
    self.norm = norm
    ...
    with tf.variable_scope(self.name + '_vars'):
      self.vars['weights'] = inits.glorot([input_dim, output_dim],
                                          name='weights')
      self.vars['u_weights'] = inits.glorot([u_dim, 1],
                                          name='u_weights')
      if self.bias:
        self.vars['bias'] = inits.zeros([output_dim], name='bias')
  ...
  def _call(self, inputs, u=None):
    x = inputs
    
    u = self.eigen_vecs
    diag_u_weights = tf.diag(tf.squeeze(self.vars['u_weights']))
    support = dot(u, diag_u_weights, sparse=False)
    support = dot(support, tf.transpose(u), sparse=False)
    ...
    if self.precalc:
      if self.u_dim == 0:
        ...
        support = dot(support, x, sparse=False)
        support = x + 0.01 * support
      # support = tf.concat((support, x), axis=1)
    else:
      support = dot(self.support, x, sparse=True)
      support = tf.concat((support, x), axis=1)

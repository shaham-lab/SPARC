...
def load_data(data_prefix, dataset_str, precalc):
  ...
  (num_data, train_adj, full_adj, feats, train_feats, test_feats, labels,
   train_data, val_data,
   test_data, train_eigen_vecs, test_eigen_vecs) = utils.load_graphsage_data(data_prefix, dataset_str)
...
def main(unused_argv):
  ...
  (_, val_features_batches, val_eigen_vecs_batches, val_support_batches, y_val_batches,
   val_mask_batches) = utils.preprocess(full_adj, test_feats, test_eigen_vecs, y_val, val_mask,
                                        np.arange(num_data),
                                        FLAGS.num_clusters_val,
                                        FLAGS.diag_lambda)

  (_, test_features_batches, test_eigen_vecs_batches, test_support_batches, y_test_batches,
   test_mask_batches) = utils.preprocess(full_adj, test_feats, test_eigen_vecs, y_test,
  ...
  pretrained_model_name = '{}{}'.format(FLAGS.dataset, 'model')  # Adjust 'model' according to your model naming convention
  ...
  if os.path.exists(pretrained_model_path):
    ...
    tf.logging.info('Pre-trained model loaded from {}'.format(pretrained_model_path))
  else:
    tf.logging.warning('Pre-trained model not found at {}. Training from scratch...'.format(pretrained_model_path))
    # Init variables
